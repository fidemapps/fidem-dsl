			...
			describe('gender', function() {
				...
				it('member with atttribute gender', function() {
					...
					try {
						...
						should(literalChoices).eql(['and', 'equal to', 'give']);
						should(otherChoices).eql(['whitespace']);
					}
			...
			describe('address', function() {
				...
				it('member with attribute address', function() {
					try {
						...
						otherChoices = helper.extractOthers(err);
						should(err.expected.length).equal(8);
						should(literalChoices).eql(['and', 'city', 'country', 'give', 'state', 'street', 'zip']);
						should(otherChoices).eql(['whitespace']);
					}
				});
			...
			describe('email', function() {
				...
				it('member with attribute email', function() {
					try {
						...
						otherChoices = helper.extractOthers(err);
						should(err.expected.length).equal(4);
						should(literalChoices).eql(['and', 'give', 'with type']);
						should(otherChoices).eql(['whitespace']);
					}
				});
