...
describe('<Unit Test>', function () {
    describe.skip('Auto-Complete Challenge Availability Rules moment-filter:', function () {
        ...
        describe('should auto-complete the moment filter', function () {
            ...
            it('member did something on', function () {
                try {
                    parser.parse('member did something on');
                } catch (error) {
                    literalChoices = helper.extractLiterals(error);
                    otherChoices = helper.extractOthers(error);

                    should(error.expected.length).equal(13);
                    should(literalChoices).eql([  'day', 'friday', 'monday', 'saturday', 'sunday', 'the', 'thursday', 'tuesday', 'wednesday', 'weekday','weekend']);
